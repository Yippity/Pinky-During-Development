ALL_COMMANDS={ # Dictionary giving specifications on all commands in alphabetical order; format is "'<command name>':['<proper usage format>','<surface-level explanation>','<in-depth explanation>']
  ...
  'audiostop': ['audiostop', 'Stops audio streaming', 'Stops audio streaming in the server where the command is used.'],
  ...
  'leavevc': ['leavevc', 'Leaves current VC', 'Leaves current VC in server where command is used.'],

  ########################
  ### joinvc
  ########################

  'joinvc':['joinvc','Joins VC with user','Joins Pinky to the same VC as the command\'s user. Must be used in the same server'],
  ...
  'playwav': ['playwav [name of song]', 'Plays selected audio from Pinky\'s .wav library','If it is available, audio streaming of the .wav file in Pinky\'s library with the given name is streamed live in VC.'],
