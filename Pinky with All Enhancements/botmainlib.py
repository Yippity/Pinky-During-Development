    ...
    try:
        ...
        if (target.id != 983212810907553803):
            ...
            pfpembed.add_field(
                name='*Disclaimer:*',
                value=
                '*Pinky\'s profile picture was created using PicsArt'
            )
...
async def announce(
        ...
        embed=None,
        title=None
):  # The bot takes user's desired message and creates an announcement out of it that is then sent by Pinky in the
    # specified channel
    try:
        if (
...
):  # Takes a specified message and creates reaction role(s) for it, both target and command must be in same channel,
    # works in conjunction with giverole and removerole
    ...
    try:
        ...
        for x in range(len(
            ...
            if (str(emojis[x]) == str(reaction.emoji)):
                ...
                role = server.get_role(roles[x])  # Fetches role object based on role ID
...
async def help(
        ...
        client,
        command=None
):  # When used with no specified command, returns list of commands; if a particular command is specified, the format of
    # the command's proper usage will be given in addition to a description of its operations
    if (command != None):
...
async def delreactionroles(
        msg
):  # Upon deletion of a message attached to reaction roles, Pinky will clear up room in its database that held
    # data necessary to the facilitation of the roles
    target = msg.message_id
    if sqlite_interface.getEntry(target):
        sqlite_interface.deleteEntry(target)
        return True
    else:
        return False
